...
namespace AbidzarFrm
{
    ...
    public sealed class SqlErrorStore : ErrorStore
    {
        ...
        protected override bool ProtectError(Guid guid)
        {
            ...
            {
                return new SqlUpdate("TbExceptions")
        ...
        protected override bool HardDeleteError(Guid guid)
        {
            using (var c = GetConnection())
            {
                return new SqlDelete("TbExceptions")
                    .Where(
                        new Criteria("[GUID]") == guid &
...
Update TbExceptions 
    ...
    Where Id In (Select Top 1 Id
                ...
                Where ErrorHash = @ErrorHash
                    ...
                    {
                        var update = new SqlUpdate("TbExceptions")
                    ...
                    c.Execute(@"
Insert Into TbExceptions ([GUID], [ApplicationName], [MachineName], [CreationDate], [Type], [IsProtected], [Host], [Url], [HTTPMethod], [IPAddress], [Source], [Message], [Detail], [StatusCode], [SQL], [FullJson], [ErrorHash], [DuplicateCount])
